...
public class StudentApi {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Name", value = "学生姓名", defaultValue = "admin", required = true),
            @ApiImplicitParam(name = "Password", value = "密码", defaultValue = "password", required = true)
    })  // 设置参数为必需
    ...
    @CrossOrigin
    @ApiImplicitParam(name = "StuId", value = "学号ID", defaultValue = "0", required = true)
    @ApiResponse(code = 100, message = "成功", response = Student.class)
    public Student getOne(@PathVariable Long StuId) {
        return sDao.findStudentByStuId(StuId);
    }
